...
const { user, listening_stats, top_genres, top_artists, top_tracks, personality } = userData;
...
/* ═══════════════════════════════════════
   HERO — Identity + Archetype compact
   ═══════════════════════════════════════ */
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <Equalizer />
            <span className="text-cream/30 text-[10px] tracking-[0.3em] uppercase">Now decoding</span>
          </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end gap-4 sm:gap-8">
        <div className="flex-1">
          ...
          <p className="text-cream/40 text-xs mt-2 max-w-sm leading-relaxed">
            Tu creuses les albums entiers, tu explores les deep cuts à 2 h du mat. Ton oreille refuse les cases.
          </p>
        </div>
...
export function StatsRow() {
  const hours = Math.round(listening_stats.total_minutes_this_year / 60);
  const stats = [
    { value: fmt(listening_stats.total_minutes_this_year), label: "Minutes", sub: `≈ ${fmt(hours)}h`, color: "text-accent" },
    { value: String(listening_stats.total_artists), label: "Artistes", sub: "89 nouveaux", color: "text-green" },
    { value: fmt(listening_stats.total_tracks), label: "Titres joués", sub: null, color: "text-cream" },
    { value: String(listening_stats.listening_streak_days), label: "Jours de streak", sub: "🔥 active", color: "text-accent" },
  ];
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {stats.map((s) => (
        <div key={s.label} className="glass p-4 md:p-5 text-center">
          <p className={`font-display text-[clamp(1.8rem,5vw,2.8rem)] leading-none ${s.color}`}>
            {s.value}
          </p>
...
/* ═══════════════════════════════════════
   TOP ARTISTS — compact list card
   ═══════════════════════════════════════ */
export function TopArtists() {
  return (
    <div className="glass p-5 md:p-6 flex flex-col h-full">
        ...
        {top_artists.map((artist, i) => (
          ...
          >
            ...
            <span className={`font-display flex-1 group-hover:text-accent transition-colors ${
              i === 0
                ? "text-accent text-xl md:text-2xl"
                : "text-cream text-lg md:text-xl"
            }`}>
              {artist.name.toUpperCase()}
            </span>

            <span className="text-cream/20 text-[10px] font-mono">
              {fmt(artist.play_count)}×
            </span>
          </a>
        ))}
      </div>
        ...
        {top_tracks.map((track) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-cream/25 text-[10px] truncate">
                {track.artist} · {track.album}
              </p>
            </div>

            <span className="text-cream/15 text-[10px] font-mono flex-shrink-0">
              {track.duration}
            </span>
          </a>
        ))}
...
/* ═══════════════════════════════════════
   GENRE DNA — horizontal genre bars
   ═══════════════════════════════════════ */
export function GenreDNA() {
  const genreColors: Record<string, string> = {
    "Indie Rock": "bg-green",
    "Electronic": "bg-purple-500",
    "Hip Hop": "bg-accent",
    "Jazz": "bg-blue-400",
    "Classical": "bg-pink-500",
    "Other": "bg-cream/30",
  };
  ...
  return (
    <div className="glass p-5 md:p-6">
      <div className="flex items-center justify-between mb-5">
        ...
        <span className="text-cream/20 text-[10px] tracking-[0.2em] uppercase">Ce qui compose ton son</span>
      </div>
        ...
        {top_genres.map((genre) => (
          <div key={genre.name} className="group">
            ...
            <div className="h-2 rounded-full bg-cream/5 overflow-hidden">
              <div
                className={`h-full rounded-full ${genreColors[genre.name] ?? "bg-cream/30"} transition-all group-hover:brightness-125`}
...
/* ═══════════════════════════════════════
   PEAK LISTENING + CTA
   ═══════════════════════════════════════ */
export function PeakAndCTA() {
  return (
    <div className="glass p-5 md:p-6 flex flex-col justify-between h-full">
      {/* Peak listening */}
      <div className="mb-6">
        ...
        <div className="flex gap-1 items-end h-12">
          {[
            ...
            <div key={t.label} className="flex-1 flex flex-col items-center gap-1">
              <div
                className={`w-full rounded-sm ${t.color} transition-all`}
                style={{ height: `${t.pct * 1.2}px` }}
              />
